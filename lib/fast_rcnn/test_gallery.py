...
def test_gallery_image(net, image, feat_blob, thresh=0.05, vis=False):
    ...
    if cfg.TEST.HAS_RPN:
        ...
        roidb = imdb.roidb
        #box_proposals = before

    _t['im_detect'].tic()
    scores, boxes, features = im_detect(net, image, feat_blob, box_proposals)
    _t['im_detect'].toc()
    print('im_detect: {:.3f}s' \
    ...
    for j in xrange(1, num_classes):
        ...
        #before = cls_boxes.astype(int)

        cls_name = 'Person';

        if vis:
           vis_detections(image, cls_name, cls_dets)
