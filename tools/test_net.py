...
def load_probe(root_dir, images_dir, gallery_size):
    ...
    for item in protoc['Query']:
        ...
        view_detection(images_dir, im_name, box)

    return protoc, images, rois, labels

def view_detection(images_dir,im_name, bbox):
    """Visual debugging of detections."""
    import matplotlib.pyplot as plt

    impath = osp.join(images_dir, im_name)
...
def evaluate(protoc, images_dir, images, result_dir, use_full_set=False):
    ...
    for i in xrange(num_probe):
	    ...
	    # print dis
            if dis[0] < dthreshold:
                bbox = det[inds[0], :4]            
                view_detection(images_dir, gallery_imname, bbox)  
...
def parse_args():
    ...
    parser.add_argument('--gallery_size',
                        ...
                        choices=[0, 50, 60,100, 500, 1000, 2000, 4000])
...
if __name__ == '__main__':
    ...
    test_net_on_gallery_set(net, imdb, args.feat_blob,
                            max_per_image=args.max_per_image, vis=True)
    ...
    # Evaluate
    evaluate(protoc, images_dir, imdb.image_index, output_dir, args.gallery_size)
